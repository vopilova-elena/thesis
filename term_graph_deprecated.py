...
from node_deprecated import NodeType, Node
from node_relation_deprecated import NodeRelationType, NodeRelation
from node_path_deprecated import NodePath
...
class TermGraph(object):
    ...
    def load_from_db(self, fname):
        ...
        for row in rows:
            ...
            node = Node(hash_node, lemmas, NodeType(int(row[3])))
            node.h = Decimal(row[9])
            node.hash_set = Hasher.get_hash_set(lemmas)
            self.hash_terms[hash_node] = node
    ...
    def save_to_db(self, fname):
        ...
        for key, value in self.hash_terms.items():
            if len(value.lemmas) == 1:
                if value.idf is None:
                    value.idf = 0
                cur.execute("INSERT INTO nodes VALUES(?, ?, ?, ?, ?, ?, ?);",
                            (str(value.hash), ' '.join(value.lemmas), 1, int(value.type),
                             value.word_count[NodeRelationType.ASS], value.node_frequency, str(value.idf)))
            else:
                cur.execute("INSERT INTO nodes VALUES(?, ?, ?, ?, ?, ?,null);",
                            (str(value.hash), ' '.join(value.lemmas), len(value.lemmas), int(value.type),
                             value.word_count[NodeRelationType.ASS], value.node_frequency))
            for rel_type in NodeRelationType:
                for h, rel in value.out_relations[rel_type].items():
                    if (rel_type == NodeRelationType.ASS) and (len(rel.end_node.lemmas) == 1):
                        cur.execute("INSERT INTO relations VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?);",
                                    (rel_type.value,
                                     str(rel.start_node.hash), str(rel.end_node.hash),
                                     rel.node_frequency, str(rel.tf), str(rel.tf_idf),
                                     str(rel.weight1), str(rel.weight2), str(rel.weight3)))
                    else:
                        cur.execute("INSERT INTO relations VALUES(?, ?, ?, ?, null, null, ?, ?, ?);",
                                    (rel_type.value, str(rel.start_node.hash), str(rel.end_node.hash), rel.node_frequency,
                                     str(rel.weight1), str(rel.weight2), str(rel.weight3)))
    ...
    def cut(self, limit):
        deleted_nodes = deleted_relations = 0
        for key, value in list(self.hash_terms.items()):
            for rel_type in NodeRelationType:
                for h, rel in list(value.out_relations[rel_type].items()):
                    if rel.weight1 < limit:
                        del value.out_relations[rel_type][h]
                        deleted_relations += 1
                for h, rel in list(value.in_relations[rel_type].items()):
                    if rel.weight1 < limit:
                        del value.in_relations[rel_type][h]
            #удаляем узлы без связей
            if sum([len(value.out_relations[rel_type]) + len(value.in_relations[rel_type])
                              for rel_type in NodeRelationType]) == 0:
                del self.hash_terms[key]
                deleted_nodes += 1
        print(len(self.hash_terms), deleted_nodes, deleted_relations)
    ...
    def get_relation_tf(self, rel):
        try:
            return round(Decimal(rel.node_frequency / rel.start_node.word_count[rel.type]), 6)
        except ZeroDivisionError:
    ...
    def calc_relation_weights(self, rel_type):
        # часть целое (нормализация не нужна)
        if rel_type == NodeRelationType.GEN:
            for key, value in self.hash_terms.items():
        ...
        if rel_type == NodeRelationType.DEF:
            for key, value in self.hash_terms.items():
        ...
        # ассоциации
        if rel_type == NodeRelationType.ASS:
    ...
    def get_node_neighborhood_recur(self, node, paths, curr_path):
        ...
        for rel_type in NodeRelationType:
            for node_hash in node.out_relations[rel_type]:
                ...
                new_path.weight = round(curr_path.weight * rel.weight3 * ATTENUATION, 6)
                new_path.relations.append(rel)
        ...
        for hash, node in self.hash_terms.items():
            ...
            for rel_type in NodeRelationType:
                ...
                for h, rel in node.in_relations[rel_type].items():
                    ...
                    total_weights3 += rel.weight3
                    count_rels += 1
            h1 = round(Decimal(total_weights1/count_rels/Decimal(log2(count_rels + 1))), 6)
            h2 = round(Decimal(total_weights2/count_rels/Decimal(log2(count_rels + 1))), 6)
            h3 = round(Decimal(total_weights3/count_rels/Decimal(log2(count_rels + 1))), 6)
            print(node.lemmas, h1, h2, h3)
            cur.execute("update nodes set h1 = ?, h2 = ?,h3 = ? where hash = ?",
                        (str(h1), str(h2),str(h3), str(hash)))
        conn.commit()
        cur.close()
